...
import { Star, ArrowRight, Clock, Users, Eye } from "lucide-react";
import { Ride } from "./mock-data";
import { ImageWithFallback } from "./figma/ImageWithFallback";
import { useRouter } from "next/navigation";
...
export function RideCard({ ride, isPast = false, onCancel, isBooked = false }: RideCardProps) {
  ...
  // Bulletproof check to see if the logged-in user is the driver of this specific ride.
  // We check multiple fallback properties depending on how the MongoDB document was populated.
  const driverId = (ride as any).driverId || (ride as any).driver?._id || (ride as any).driver;
  const isOwnRide = Boolean(user?._id && driverId === user._id);
          ...
          <div className="flex items-center gap-2">
            ...
            {!isPast && !isBooked && (
              <button
                ...
                className={`px-4 py-1.5 rounded-xl text-[13px] font-semibold transition-colors flex items-center gap-1.5 active:scale-95 ${
                  isOwnRide 
